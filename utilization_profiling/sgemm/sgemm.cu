...
// adapted from
// https://github.com/ROCmSoftwarePlatform/rocBLAS/blob/rocm-3.0/clients/include/rocblas_init.hpp#L42
...
// C(m,n) = A(m,k) * B(k,n)
void gpu_blas_mmul(cublasHandle_t handle, const float *A, const float *B,
  ...
  cublasStatus_t err = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k,
...
int def(int value, int reps, int device) {
  ...
  // cudaMemcpy(d_A,h_A,nr_rows_A * nr_cols_A *
  // sizeof(float),cudaMemcpyHostToDevice); cudaMemcpy(d_B,h_B,nr_rows_B *
  // nr_cols_B * sizeof(float),cudaMemcpyHostToDevice);
  ...
  cudaMemcpyAsync(d_A, h_A, nr_rows_A * nr_cols_A * sizeof(float),
                  cudaMemcpyHostToDevice, computeStream);
  cudaMemcpyAsync(d_B, h_B, nr_rows_B * nr_cols_B * sizeof(float),
                  cudaMemcpyHostToDevice, computeStream);
  // std::cout << "A =" << std::endl;
  ...
  memcpy_timer.Stop();
  // std::cout << "Memcpy Runtime (ms) = " << memcpy_timer.Elapsed() <<
  // std::endl;
  ...
  for (int i = 0; i < reps; i++) {
    ...
    cudaStreamSynchronize(computeStream);
    //		timer.Stop();
    // std::cout << "Kernel " << i << " Runtime = " << timer.Elapsed() <<
    // std::endl;
  }
  ...
  cudaMemcpyAsync(h_C, d_C, nr_rows_C * nr_cols_C * sizeof(float),
