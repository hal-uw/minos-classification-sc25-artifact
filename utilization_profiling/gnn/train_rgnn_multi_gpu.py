...
def evaluate(model, dataloader, current_device, rank, world_size, epoch_num):
    ...
    with torch.no_grad():
        ...
        torch.distributed.all_reduce(
            acc_tensor, op=torch.distributed.ReduceOp.SUM)
...
def run_training_proc(rank, world_size,
                      ...
                      evaluate_on_epoch_end, checkpoint_on_epoch_end, ckpt_steps, ckpt_path, warmup_iters, running_iters):
    if rank == 0:
        if ckpt_steps > 0:
            ckpt_dir = create_ckpt_folder(
    ...
    model = DistributedDataParallel(model,
                                    device_ids=[
                                        current_device.index] if with_gpu else None,
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for batch in train_loader:
            drv.start_profiler()
            idx += 1
            nvtx.range_push(f'iteration_{idx}')
            if idx > running_iters:
                nvtx.range_pop()
                drv.stop_profiler()
                print("stop the whole training process")
                exit(0)

            # print("start profiling...")
            # if idx > running_iters:
            #     # stop the whole training process
            #     drv.stop_profiler()
            #     exit(0)
            # if idx == warmup_iters:
            #     print("start profiling...")
    ...
    print("Total time taken " +
          str(datetime.timedelta(seconds=int(time.time() - training_start))))
...
if __name__ == '__main__':
    ...
    root = osp.join(osp.dirname(osp.dirname(
        osp.dirname(osp.realpath(__file__)))), 'data', 'igbh')
    ...
    parser.add_argument('--running_iters', type=int,
                        default=10, help="Number of running iterations.")
    args = parser.parse_args()
    args.with_gpu = (not args.cpu_mode) and torch.cuda.is_available()
    assert args.layout in ['COO', 'CSC', 'CSR']
    # set environment variables
    # CUDA_VISIBLE_DEVICES = 0
    # todo add a argument to set the GPU
    ...
    #                value=world_size*args.train_batch_size)
    ...
    torch.multiprocessing.spawn(
        ...
        args=(world_size, args.hidden_channels, args.num_classes, args.num_layers,
              ...
              args.ckpt_steps, args.ckpt_path, args.warmup_iters, args.running_iters),
        nprocs=world_size,
        join=True
    )