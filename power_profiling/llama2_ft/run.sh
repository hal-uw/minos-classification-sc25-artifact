...
tune download meta-llama/Llama-2-7b-hf \
  --output-dir ../../datasets/model_cache/data/ \
  --hf-token $HF_TOKEN
...
echo "Starting Llama 2 finetuning profiling with ROCProfiler..."
...
for set_name in "${!COUNTER_FILES[@]}"; do
    ...
    python3 $WRAPPER --cmd "$APP_CMD" --gpus 0 --prefix "${set_name}" --counters_file "$COUNTER_FILE"
