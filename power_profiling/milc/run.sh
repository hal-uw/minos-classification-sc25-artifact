...
MILC_INPUT="../../datasets/MILC/benchmark.in"
if [ ! -f "$MILC_INPUT" ]; then
    echo "Error: Input file not found: $MILC_INPUT"
    exit 1
fi
...
echo "Starting MILC profiling with ROCProfiler..."
# get the host name
hostname=$(hostname)
...
for set_name in "${!COUNTER_FILES[@]}"; do
    ...
    python3 $WRAPPER --cmd "$MILC_CMD" --gpus 0 --prefix "${prefix}" --counters_file "$COUNTER_FILE"
