...
DEEPMD_INPUT="../../datasets/DeePMD/water_se_e2_a_input.json"
if [ ! -f "$DEEPMD_INPUT" ]; then
    echo "Error: Input file not found: $DEEPMD_INPUT"
    exit 1
fi

# Set running commad
DEEPMD_CMD="dp --pt train $DEEPMD_INPUT"
...
echo "Starting deepmd profiling with ROCProfiler..."
# get the host name
...
for set_name in "${!COUNTER_FILES[@]}"; do
    ...
    python3 $WRAPPER --cmd "$DEEPMD_CMD" --gpus 0 --prefix "${prefix}" --counters_file "$COUNTER_FILE"
